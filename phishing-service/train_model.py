...
def extract_sender_features(sender):
    ...
    features["sender_tld_unusual"] = int(
        d.endswith((".tk", ".xyz", ".top", ".club", ".info")) or d.endswith(".ru")
    )
    return features
...
def make_features(df):
    ...
    df["has_login_token"] = df["body"].str.contains(
        r"\b(verify|confirm|login|password|secure)\b", na=False, flags=re.I
    ).astype(int)
    df["first_link_entropy"] = df["body"].apply(
        lambda t: host_entropy_from_url(extract_links(t)[0]) if extract_links(t) else 0.0
    )
...
if "label" not in df.columns:
    raise ValueError("CSV must contain column 'label' (0 or 1).")
...
joblib.dump(
    {
        ...
        "scaler": scaler,
        "clf": clf,
        "meta": {"free_domains": list(FREE_EMAIL_DOMAINS)},
    },
    "phish_model.joblib",
)
print("✅ Model saved successfully as phish_model.joblib")
