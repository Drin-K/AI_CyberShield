...
with LOCK:
info = MESSAGES.setdefault(mid, {'chunks': {}, 'total': int(total), 'last_seen': time.time()})
...
del MESSAGES[mid]
return jsonify({'status': 'complete', 'message_id': mid, 'reconstructed_b64': reconstructed_b64}), 200


return jsonify({'status': 'received', 'message_id': mid, 'received_chunks': len(info['chunks'])}), 202
...
return jsonify({'status': 'in_progress', 'received_chunks': len(info['chunks']), 'total': info['total']}), 200
...
return jsonify({'status': 'not_found'}), 404
if len(info['chunks']) != info['total']:
return jsonify({'status': 'incomplete', 'received_chunks': len(info['chunks']), 'total': info['total']}), 202
parts = [info['chunks'][i] for i in range(info['total'])]
full = b''.join(parts)
reconstructed_b64 = base64.b64encode(full).decode()
return jsonify({'status': 'complete', 'message_id': message_id, 'reconstructed_b64': reconstructed_b64}), 200
